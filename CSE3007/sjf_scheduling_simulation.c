...
void schedule_sjf(PROCESS *processes, int p_count) {
    ...
    for(index_a = 0; index_a < p_count - 1; index_a++) {
        for(index_b = 0; index_b < p_count - index_a - 1; index_b++) {
            if((processes + index_b)->bt > (processes + index_b + 1)->bt) {
...
int main() {
    ...
    // schedule / sort on basis of SJF
    schedule_sjf(processes, count);
