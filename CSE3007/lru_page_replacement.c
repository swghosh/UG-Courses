...
struct frame {
    ...
    char data[PAGE_SIZE];
    int access_time; // higher the more recently used
};
...
int locate_lru(FRAME *frames, int f_count) {
    int iter_index, k_min, k_index;
    k_index = -1;
    k_min = INT_MAX;
    for(iter_index = 0; iter_index < f_count; iter_index++) {
        if(frames[iter_index].access_time < k_min) {
            k_min = frames[iter_index].access_time;
            k_index = iter_index;
        }
    }
    return k_index;
...
void lru_entry(FRAME *frames, int f_count, PAGE *page, int access_time) {
    ...
    if(allocated_count == f_count) {
        entry_id = locate_lru(frames, f_count);
    }
    else {
    ...
    frames[entry_id].access_time = access_time;
    copy_data(page, &frames[entry_id]);
    printf("Allocated page %d to frame %d.\n", frames[entry_id].page_id, 
        frames[entry_id].id);
}

bool is_page_present(FRAME *frames, int f_count, int page_id, int access_time) {
    int iter_index;
    for(iter_index = 0; iter_index < f_count; iter_index++) {
        if(frames[iter_index].is_allocated && frames[iter_index].page_id == page_id) {
            frames[iter_index].access_time = access_time; // update access time
            return true;
        }
    }
    return false;
...
int main() {
    ...
    puts("Using LRU page replacement.");
    fault_count = 0;
    print_frames(frames, f_count);
    for(iter_index = 0; iter_index < r_count; iter_index++) {
        printf("Step %d:\n", iter_index + 1);
        extra_id = page_requirements[iter_index];
        if(!is_page_present(frames, f_count, extra_id, iter_index)) {
            lru_entry(frames, f_count, pages + extra_id, iter_index);
            fault_count++;
        }
        print_frames(frames, f_count);
    }
